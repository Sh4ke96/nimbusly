...
<body style="font-family:system-ui,sans-serif;background:#f6f4f0;padding:24px">
  <div style="max-width:520px;margin:0 auto;background:#fff;border:1px solid #e5e0d8;padding:32px">
    ...
    <h1 style="font-size:22px;margin:16px 0 8px">${t.digestEmailHeading}</h1>
    <p style="color:#4a4a4a;line-height:1.5">${t.digestEmailIntro}</p>
    ${attentionBlock}
    ${params.activitySections.length > 0 ? `<h2 style="font-size:16px;margin:20px 0 8px">${t.digestActivityHeading}</h2>${activityBlocks}` : ""}
